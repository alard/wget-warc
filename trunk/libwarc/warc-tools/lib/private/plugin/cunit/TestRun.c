                 ...
                 CU_pTest pTest)
{
  ...
  if (NULL != szFileName) {
    size_t l = strlen(szFileName);
    pFailureNew->strFileName = (char*)CU_MALLOC(l + 1);
    if(NULL == pFailureNew->strFileName) {
      CU_FREE(pFailureNew);
      return;
    }
    strncpy(pFailureNew->strFileName, szFileName, l);
  }

  if (NULL != szCondition) {
    size_t l = strlen(szCondition);
    ...
    strncpy(pFailureNew->strCondition, szCondition,l);
  }
